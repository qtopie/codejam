...
7. [相邻概念辨析：BIT vs 二分查找、线段树 vs 跳表](#7-相邻概念辨析bit-vs-二分查找线段树-vs-跳表)
8. [高频避坑与边界清单 (Gotchas)](#8-高频避坑与边界清单-gotchas)
...
## 7. 相邻概念辨析：BIT vs 二分查找、线段树 vs 跳表

面试中容易把「树状数组的 O(log n)」与「二分查找的 O(log n)」、「线段树」与「跳表」混淆，这里做一个概念边界澄清。

### 7.1 BIT 单点更新 vs 二分查找

二者是**不同层次**的东西——一个是数据结构，一个是算法：

| 对比维度 | 二分查找 | BIT 单点更新 |
| :--- | :--- | :--- |
| 本质 | **算法**（在有序数组上找值） | **数据结构**（维护动态前缀和） |
| 前提 | 数据**有序/单调**、静态 | 无顺序要求，支持**动态修改** |
| 修改代价 | 改一个元素 $O(n)$（要维持有序） | 单点改 $O(\log n)$ |
| 查询 | 查某个值是否存在 $O(\log n)$ | 查任意前缀和 $O(\log n)$ |
| 典型场景 | 有序数组定位 | 动态数组区间和 |

**为什么都会是 O(log n)？** BIT 的 `i += i & -i` 上升 / 下降本质是**按二进制位跳**，可理解为一种「结构化的二分」。甚至可以在 BIT 上做二分（如「找前缀和 ≥ k 的最小下标」，配合 `sort.Search` 实现）。

**关键区别**：二分查找**只读、不能改**；BIT 的核心价值是**读写都是 O(log n)**——用二分，数组一旦变化就要 O(n) 重建；用 BIT，改一个点只动 log n 个桶。

### 7.2 线段树 vs 跳表 Skiplist

| 对比维度 | 线段树 | 跳表 Skiplist |
| :--- | :--- | :--- |
| 存储 | 完全二叉树，$4n$ 数组 | 多层链表，随机指针 |
| 结构 | **静态**（建树后大小固定） | **动态**（节点随时增删） |
| 核心操作 | **区间聚合**（sum/min/max）+ lazy 区间更新 | **有序操作**（按 key 找前驱/后继、范围遍历） |
| 区间求和/最值 | $O(\log n)$ ✔ | ✗（只能遍历范围内元素） |
| 单点修改 | $O(\log n)$ | $O(\log n)$ |
| 确定性 | 确定 $O(\log n)$ | **随机化**，期望 $O(\log n)$ |
| 典型场景 | 刷题、区间统计问题 | Redis ZSET、LevelDB memtable、并发无锁结构 |

**一句话总结**：线段树是「**按区间分治**」的静态树，擅长区间聚合查询/更新；跳表是「**按 key 排序**」的动态有序结构，擅长增删查与范围遍历，但不擅长区间聚合。跳表更像**平衡 BST（如红黑树）的替代品**（有序 map），而线段树更像「**支持区间运算的动态数组**」——选型取决于你要查的是「区间和/最值」（线段树）还是「有序范围/单 key」（跳表）。
